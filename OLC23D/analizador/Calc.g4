...
asignacion returns[interfaces.Instruccion instr]
    : ID '=' expresion                                                     {$instr = asignacion.NewAsignacion($ID.text, $expresion.expr)}
;
...
expr_log returns[interfaces.Expresion expr]
    : opIz = expr_log AND opDe = expr_log {$expr = expresion.NewOperacion($opIz.expr,"&&",$opDe.expr,false)}
    | opIz = expr_log OR opDe = expr_log {$expr = expresion.NewOperacion($opIz.expr,"||",$opDe.expr,false)}
    | expr_rel  {$expr = $expr_rel.expr}
;
